...
const API_CONFIG: ApiConfig = {
  baseUrl: process.env.EXPO_PUBLIC_API_URL || 'https://dalmas.pythonanywhere.com',
  timeout: 10000,
  retries: 3
};
  ...
  private playWebRingtone(): void {
    try {
      ...
      audio.play().then(() => {
        ...
        console.warn('Web audio play failed:', webError);
      });
...
export default function ProfessionalDashboardScreen() {
  ...
  // Handle call acceptance - REDIRECTS TO PROFESSIONAL-CALL PAGE
  const handleAcceptCall = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Navigate to professional-call page for actual call handling
        router.push({
          ...
          params: { 
            ...
            callType: incomingCall.mode === 'audio' ? 'voice' : incomingCall.mode === 'video' ? 'video' : 'chat',
            isIncomingCall: 'true',
    ...
    <SafeAreaView style={styles.container}>
      ...
      {showCallNotification && incomingCall && (
        <View style={styles.callNotification}>
          <TouchableOpacity 
            style={styles.callNotificationContent}
